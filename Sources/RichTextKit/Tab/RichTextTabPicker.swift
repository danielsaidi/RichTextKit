//
//  RichTextTabPicker.swift
//  RichTextKit
//
//  Created by James Bradley on 2022-03-04.
//  Copyright © 2023 James Bradley. All rights reserved.
//

import SwiftUI

/**
 This picker can be used to pick a rich text tab.

 This view returns a plain SwiftUI `Picker` view that can be
 styled and configured in all ways supported by SwiftUI.
 */
public struct RichTextTabPicker: View {

    /**
     Create a rich text tab picker.

     - Parameters:
       - selection: The binding to update with the picker.
       - values: The pickable alignments, by default all available alignments.
     */
    public init(
        selection: Binding<RichTextTab>,
        values: [RichTextTab] = RichTextTab.allCases
    ) {
        self._selection = selection
        self.values = values
    }

    let values: [RichTextTab]

    @Binding
    private var selection: RichTextTab
    
    public var body: some View {
        Picker("", selection: $selection) {
            ForEach(RichTextTab.allCases) {
                $0.icon.tag($0)
            }
        }.labelsHidden()
    }
}
