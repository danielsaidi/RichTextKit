...
class RichTextDataReaderWriterTests: XCTestCase {
    ...
    func testCanGenerateAndParseArchivedData() {
        ...
        guard let data = try? string.richTextArchivedData() else { return XCTFail() }
    ...
    func testCanGenerateAndParsePlainTextData() {
        let string = NSAttributedString(string: "foo bar baz")
        guard let data = try? string.richTextPlainTextData() else { return XCTFail() }
    ...
    func testCanGenerateAndParseRtfData() {
        let string = NSAttributedString(string: "foo bar baz")
        guard let data = try? string.richTextRtfData() else { return XCTFail() }
    ...
    func testDataForFormatCanGenerateAndParseArchivedData() {
        let string = NSAttributedString(string: "foo bar baz")
        guard let data = try? string.richTextData(for: .archivedData) else { return XCTFail() }
    ...
    func testDataForFormatCanGenerateAndParsePlainTextData() {
        let string = NSAttributedString(string: "foo bar baz")
        guard let data = try? string.richTextData(for: .plainText) else { return XCTFail() }
    ...
    func testDataForFormatCanGenerateAndParseRtfData() {
        let string = NSAttributedString(string: "foo bar baz")
        guard let data = try? string.richTextData(for: .rtf) else { return XCTFail() }
